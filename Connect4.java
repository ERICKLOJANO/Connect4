...
public class Connect4 extends JFrame {
		...
		public Connect4() {
			...
			top = new JLabel("CONNECT4");
			top.setFont(top.getFont().deriveFont(25f));
			
			bottom = new JLabel("PLAYERS" + "     "+ player1.getName() + ": " + numWins1 + "     " + player2.getName() + ": " + numWins2);
			bottom.setFont(bottom.getFont().deriveFont(15f));
			middle = new JLabel(" Total games played: " + totalGames);
			middle.setFont(middle.getFont().deriveFont(15f));
			
			jpBoard = new CFOUR();
			scoreBoard.add(BorderLayout.NORTH, top);
			scoreBoard.add(BorderLayout.CENTER, bottom);
			scoreBoard.add(BorderLayout.SOUTH, middle);
		...
		private class CFOUR extends JPanel implements GameBoard, PlayerInterface, ActionListener {
			...
			public void actionPerformed(ActionEvent e) {
				...
				//checks if a certain player wins, and then adds a win to the sb
				if(isWinner() == true){
					...
					int reply = JOptionPane.showConfirmDialog(null, "Want to play again?", "Hmmm?", JOptionPane.YES_NO_OPTION);
					if(reply == JOptionPane.YES_OPTION) {
						clearBoard();
					}else {
						System.exit(0);
			...
			//checks if a player wins in different methods
			@Override
